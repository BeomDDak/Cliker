...
public class RunBank : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 시간초 계산해주기
        timeremain = 20 + Goods.gm.quickfeet.value;
    ...
    void Update()
    {
        ...
        if (startpanel.activeSelf == false && liePanel.activeSelf == false)
        {
            ...
            coinCountTxt.text = "수량 : " + coinCount.ToString();
    ...
    void SuccessOrNot()
    {
        ...
        if (success <= successProbability)
        {
            ...
            //아이템 경험치 올려줌
            Goods.gm.coin.count += coinCount;
            Goods.gm.cash.count += cashCount;
            Goods.gm.bitcoin.count += bitcoinCount;
            Goods.gm.characterLevel += 1500;
        }
        else
        {
    ...
    void Click()
    {
        ...
        if(bagWeight < Goods.gm.bagWeight)
        {
            switch (i)
            {
                case int x when (x >= 0 && x < 776 - Goods.gm.judgment.value):
                    ...
                    coinCount++;
                    break;
                case int x when (x >= 776 - Goods.gm.judgment.value && x <= 999):
            ...
            bagWeight = (int)(coinCount + cashCount + bitcoinCount);
        }
        else
        {
